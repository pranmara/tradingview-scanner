...
class CompositeMarketDataProvider:
    """MCP first, then direct REST fallbacks (Binance for crypto, Yahoo for stocks)."""
    ...
    async def get_ohlcv(self, asset: AssetInfo, timeframe: Timeframe, limit: int) -> OHLCV:
        ...
        if asset.is_crypto:
            chain.append(("binance", lambda: self._binance.get_ohlcv(asset.binance_symbol, timeframe, limit)))
    ...
    async def get_ohlcv_history(self, asset: AssetInfo, timeframe: Timeframe, bars: int) -> OHLCV:
        """Deep history for backtests. Binance pages; other sources return what a single request allows."""
        if asset.is_crypto:
            return await self._binance.get_ohlcv_history(asset.binance_symbol, timeframe, bars)
    ...
    async def health(self) -> dict[str, str]:
        ...
        mcp, binance, session = await asyncio.gather(mcp_status(), self._binance.ping(), session_status())
        return {
